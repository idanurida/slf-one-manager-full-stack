...
export default function AwaitingApprovalPage() {
    ...
    useEffect(() => {
        if (mounted && typeof window !== 'undefined') {
            ...
            if (r === 'registered') {
                ...
                if (email || fullName) {
                    setTempUser({ email, full_name: fullName, role });
                }
            }
        }
    }, [mounted]);
    ...
    const getStatusInfo = () => {
        ...
        const hasNullStatus = profile.status === null || profile.status === undefined;
        const hasNullApproved = profile.is_approved === null || profile.is_approved === undefined;
        // Fix: Ensure profile exists before checking properties to avoid "undefined === undefined" true positive
        const isExistingUser = profile && hasNullStatus && hasNullApproved;
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-medium text-foreground">{user?.email || tempUser?.email || 'Check your inbox'}</span>
                                </div>

...
import {
  ...
  LayoutDashboard, FolderOpen, Users, Settings, LogOut, Moon, Sun, Building2, Home, Mail, PlusCircle, Zap, ChevronRight, Bell, Menu, CalendarDays, BarChart3
} from "lucide-react";
...
export default function HeadConsultantApprovalsPage() {
  ...
  return (
    <DashboardLayout hideSidebar={false} showHeader={true}>
      <div className="min-h-screen flex flex-col bg-background-light dark:bg-background-dark font-bold text-slate-900 dark:text-slate-100 transition-colors duration-300">
          ...
          <div className="mx-auto max-w-7xl flex flex-col gap-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div className="flex flex-col gap-1">
                <h1 className="text-3xl md:text-4xl font-display font-bold text-slate-900 dark:text-white tracking-tight">Validasi laporan</h1>
                <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base">Tinjau dan berikan persetujuan akhir pada dokumen teknis sebelum penerbitan sertifikat.</p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={handleRefresh}
                  disabled={loading}
                  className="flex items-center justify-center gap-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white font-bold text-xs px-6 py-3 rounded-xl shadow-sm transition-all hover:bg-gray-50 disabled:opacity-50"
                >
            ...
            <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h4 className="text-xs font-bold text-primary">Saring berkas laporan</h4>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="relative md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-slate-100 dark:border-slate-800 bg-gray-50/50 dark:bg-black/20 py-3 pl-12 pr-4 text-sm font-semibold focus:ring-2 focus:ring-primary outline-none transition-all placeholder-slate-400/50 text-slate-900 dark:text-white"
                    />
                  </div>
                </div>
                <div className="relative">
                  <span className="absolute -top-2 left-3 px-1 bg-white dark:bg-slate-900 text-xs font-bold text-primary z-10">Status persetujuan</span>
                  <select
                    value={statusFilter}
                ...
                <div className="relative">
                  <span className="absolute -top-2 left-3 px-1 bg-white dark:bg-slate-900 text-xs font-bold text-primary z-10">Entitas proyek</span>
                  <select
                    value={projectFilter}
            ...
            <div className="rounded-[2.5rem] border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl shadow-gray-200/50 dark:shadow-none overflow-hidden transition-all duration-300">
              <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 bg-gray-50/30 dark:bg-black/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Antrean laporan ({filteredReports.length})</h3>
                    <p className="text-[10px] font-bold text-slate-500">Membutuhkan verifikasi HC</p>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/50 dark:bg-black/5">
                      <th className="px-8 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">Identitas laporan</th>
                      <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">Informasi laporan</th>
                      <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">Proyek terkait</th>
                      <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">Progress review</th>
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                    {loading ? (
                      <tr><td colSpan="5" className="px-8 py-20 text-center"><div className="flex flex-col items-center gap-3"><RefreshCw className="w-8 h-8 text-primary animate-spin" /><span className="text-xs font-bold text-slate-500">Sinkronisasi dokumen...</span></div></td></tr>
                    ) : filteredReports.length === 0 ? (
                      <tr><td colSpan="5" className="px-8 py-20 text-center flex flex-col items-center justify-center"><div className="h-20 w-20 flex items-center justify-center rounded-full bg-gray-50 dark:bg-white/5 mb-4"><FolderOpen size={40} className="text-slate-500/20" /></div><p className="font-bold text-sm text-slate-500">Tidak ada laporan menunggu</p></td></tr>
                    ) : (
                      filteredReports.map(report => (
                        <tr key={report.id} className="group hover:bg-primary/5 transition-all duration-300">
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-3">
                              <div className="h-10 w-10 flex-shrink-0 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-slate-500 group-hover:bg-primary group-hover:text-white transition-all">
                                <FileText size={18} />
                              </div>
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs font-bold text-slate-500">Diperbarui: {formatDateSafely(report.updated_at)}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-slate-900 dark:text-gray-200 tracking-tight">{report.project_name}</span>
                              <span className="text-[10px] font-bold text-primary">Proyek utama</span>
                            </div>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-2">
                              <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-[#7c3aed] to-[#a855f7] flex items-center justify-center text-[10px] text-white font-bold shadow-sm">
                                {(report.inspector_name || 'I')[0]}
                              </div>
                              <div className="flex flex-col">
                                <span className="text-xs font-bold text-slate-900 dark:text-gray-300 leading-none">{report.inspector_name}</span>
                                <span className="text-[9px] font-medium text-slate-500">{report.inspector_specialization}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-8 py-6">
                          ...
                          <td className="px-8 py-6 text-right">
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="h-10 w-10 flex items-center justify-center rounded-xl bg-gray-50/50 dark:bg-white/5 text-slate-500 hover:bg-primary hover:text-white transition-all shadow-sm"
                              ...
                              {(report.status === 'approved_by_admin_lead' || report.status === 'head_consultant_review') && (
                                <>
                                  <button
                                    ...
                                    className="h-10 w-10 flex items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all shadow-sm"
                                  ...
                                  <button
                                    ...
                                    className="h-10 w-10 flex items-center justify-center rounded-xl bg-red-500/10 text-red-600 hover:bg-red-500 hover:text-white transition-all shadow-sm"
                                    title="Tolak Laporan"
                                  >
                                    <X size={18} />
                                  </button>
...
function StatusBadge({ status }) {
  const configs = {
    ...
    'approved': { label: 'Disetujui (OK)', class: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
    'slf_issued': { label: 'Sertifikasi terbit', class: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
    'rejected': { label: 'Ditolak/revisi', class: 'bg-red-500/10 text-red-600 border-red-500/20' },

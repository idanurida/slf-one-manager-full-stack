...
import { X, MapPin, FileText, AlertCircle, Users, UserCheck } from "lucide-react";
...
const BulkScheduleInspections = ({ 
  ...
  const validateForm = () => {
    ...
    if (selectedInspectors.length === 0) {
      newErrors.inspectors = 'Pilih minimal satu inspector';
    }
  ...
  const handleInspectorSelection = (inspectorId, checked) => {
    if (checked) {
      setSelectedInspectors(prev => [...prev, inspectorId]);
    } else {
      setSelectedInspectors(prev => prev.filter(id => id !== inspectoratorId));
    }
    
    // Clear inspectors error
    if (errors.inspectors) {
      setErrors(prev => ({
        ...prev,
        inspectors: ''
      }));
  ...
  // Handle cancel
  const handleCancel = () => {
    console.log('Tombol batal diklik - BulkScheduleInspections');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (errors.length > 0) {
        console.error('Some inspections failed:', errors);
        toast.warning(`${successes.length} jadwal berhasil, ${errors.length} gagal`);
      } else {
        toast.success(`Berhasil membuat ${successes.length} jadwal inspeksi!`);
      }
      ...
      if (onSchedulesCreated && typeof onSchedulesCreated === 'function') {
        onSchedulesCreated(successes.map(s => s.data));
      } else {
        console.warn('onSchedulesCreated prop tidak tersedia');
      }

    } catch (error) {
      console.error('Error creating inspection schedules:', error);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center pb-4 border-b">
        <div>
          <p className="text-muted-foreground">
            Jadwalkan inspeksi untuk multiple inspektor pada proyek yang sama
          </p>
        </div>
        <Button
          onClick={handleCancel}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="hover:shadow-md transition-shadow duration-200">
          <CardContent className="space-y-4 pt-6">
            <div className="space-y-3">
              <Label className="text-base font-semibold">Pilih Inspektor *</Label>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {selectedInspectors.length} inspektor terpilih
                </span>
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="select-all"
                    checked={selectedInspectors.length === inspectors.length && inspectors.length > 0}
                    onCheckedChange={handleSelectAll}
                    className="hover:border-primary transition-colors"
                  />
                  <Label htmlFor="select-all" className="text-sm text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
                    Pilih Semua
                  </Label>
                </div>
              </div>

              <div className="grid grid-cols-1 gap-2 max-h-[200px] overflow-y-auto border border-border rounded-md p-3 hover:border-primary/50 transition-colors">
                {inspectors.map((inspector) => (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleInspectorSelection(inspector.id, checked)}
                      className="hover:border-primary transition-colors"
                    />
        ...
        <Card className="hover:shadow-md transition-shadow duration-200">
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              <Textarea
                id="notes"
                placeholder="Tambahkan catatan atau instruksi khusus untuk semua inspektor (opsional)..."
              ...
              <p className="text-sm text-muted-foreground">
                Catatan ini akan dilihat oleh semua inspector sebelum melakukan inspeksi.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Schedule Preview */}
        {(selectedProject || selectedInspectors.length > 0 || formData.start_date || formData.end_date) && (
          <Card className="hover:shadow-md transition-shadow duration-200">
            <CardContent className="pt-6">
              ...
              <div className="space-y-3 mt-3">
                ...
                {selectedInspectors.length > 0 && (
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-sm font-medium">Jumlah Inspektor:</span>
                    <span className="text-sm text-right">{selectedInspectors.length} inspektor</span>
                  </div>
                )}
                {formData.start_date && (
                  <div className="flex justify-between items-center py-2 border-b">
        ...
        <div className="flex justify-end gap-3 pt-6 border-t">
          ...
          <Button 
            type="submit" 
            disabled={loading || selectedInspectors.length === 0}
          ...
          >
            {loading ? (
              ...
              <>
                ...
                Buat {selectedInspectors.length} Jadwal
              </>
            )}
          </Button>
        </div>
...
export default BulkScheduleInspections;

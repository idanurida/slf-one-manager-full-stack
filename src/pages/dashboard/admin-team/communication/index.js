...
// FILE: src/pages/dashboard/admin-team/communication/index.js
...
export default function AdminTeamCommunicationInbox() {
    const router = useRouter();
    const { user, loading: authLoading, isAdminTeam } = useAuth();
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            // 1. Get Project IDs where this user is assigned
            const { data: teamRes, error: teamResErr } = await supabase
                .from('project_teams')
                .select('project_id')
                .eq('user_id', user.id);

            if (teamResErr) throw teamResErr;

            const projectIds = [...new Set((teamRes || []).map(a => a.project_id))];
    ...
    useEffect(() => {
        if (user && isAdminTeam) fetchData();
    }, [user, isAdminTeam, fetchData]);
    ...
    if (authLoading || (user && !isAdminTeam)) return null;

      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <motion.div variants={itemVariants}>
            ...
            <p className="text-slate-500 font-medium">Jadwal inspeksi, meeting, dan deadline.</p>
          </motion.div>
          <motion.div variants={itemVariants} className="flex gap-3">
            <Button size="sm" onClick={handleOpenCreateDialog} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold uppercase tracking-widest text-xs h-10 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="wait">
            {loading ? (
              [1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-48 bg-slate-100 dark:bg-slate-800 rounded-[2rem] animate-pulse" />)
              ...
              <div className="col-span-full py-20 text-center">
                ...
                <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white mb-2">Tidak ada jadwal</h3>
                ...
                >
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleOpenEditDialog(schedule)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition-colors">
                        <Edit size={16} />
                      </button>
                  ...
                  <div className="mb-6 relative z-10">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <Badge variant="outline" className="bg-slate-50 dark:bg-slate-800 border-transparent text-[10px] uppercase font-bold tracking-wider text-slate-500">
                        {schedule.project_name}
                      </Badge>
                      <Badge className={`${getStatusColor(schedule.status)} border-none text-[10px] uppercase font-bold tracking-wider`}>
        ...
        <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
          <DialogContent className="rounded-[2rem] border-none bg-white dark:bg-[#1e293b] sm:max-w-[600px]">
            <DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4 py-4">
              <div className="col-span-2">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Judul</Label>
                <Input
                  value={formData.title}
                  onChange={e => setFormData({ ...formData, title: e.target.value })}
                  className="rounded-xl bg-slate-50 dark:bg-slate-900 border-none"
                />
              </div>
              <div className="col-span-2 md:col-span-1">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Proyek</Label>
                <Select value={formData.project_id} onValueChange={v => setFormData({ ...formData, project_id: v })}>
                  <SelectTrigger className="rounded-xl bg-slate-50 dark:bg-slate-900 border-none"><SelectValue /></SelectTrigger>
                  <SelectContent className="rounded-xl">
              ...
              <div className="col-span-2 md:col-span-1">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Tipe</Label>
              ...
              <div className="col-span-2 md:col-span-1">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Tanggal</Label>
                <Input
                  type="datetime-local"
              ...
              <div className="col-span-2 md:col-span-1">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Status</Label>
              ...
              <div className="col-span-2">
                <Label className="uppercase text-[10px] font-bold tracking-widest text-slate-500 mb-1">Ditugaskan Ke</Label>
                <Select value={formData.assigned_to} onValueChange={v => setFormData({ ...formData, assigned_to: v })}>
                  <SelectTrigger className="rounded-xl bg-slate-50 dark:bg-slate-900 border-none"><SelectValue placeholder="Opsional" /></SelectTrigger>
                  <SelectContent className="rounded-xl">
            ...
            <DialogFooter>
              ...
              <Button onClick={editingSchedule ? handleUpdateSchedule : handleCreateSchedule} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold">Simpan</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
...
const getScheduleColor = (type, bgOnly = false) => {
  switch (type) {
    case 'inspection': return bgOnly ? 'bg-orange-100 text-orange-600' : '';
    case 'meeting': return bgOnly ? 'bg-blue-100 text-blue-600' : '';
    case 'report_review': return bgOnly ? 'bg-purple-100 text-purple-600' : '';
    default: return bgOnly ? 'bg-slate-100 text-slate-600' : '';
  }
}

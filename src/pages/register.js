...
export default function RegisterPage() {
    ...
    // START: PENAMBAHAN STATE BARU UNTUK VALIDASI FIELD
    const [fieldErrors, setFieldErrors] = useState({});
    // END: PENAMBAHAN STATE BARU UNTUK VALIDASI FIELD
    ...
    const handleChange = (e) => {
        ...
        if (error) setError('');
        // Hapus error spesifik field saat user mulai mengetik
    ...
    // START: FUNGSI VALIDASI BARU UNTUK STEP 1
    const validateStep1 = () => {
        const errors = {};
        ...
        if (!formData.email.trim() || !emailRegex.test(formData.email)) {
            errors.email = 'Format email tidak valid atau wajib diisi.';
        }
        if (!formData.role) {
            errors.role = 'Role/Peran wajib dipilih.';
        }
        // Wajib isi specialization jika role adalah inspector
        if (formData.role === 'inspector' && !formData.specialization) {
            errors.specialization = 'Spesialisasi wajib diisi untuk Inspector.';
        }

        setFieldErrors(errors);
        
        // Hapus error umum jika validasi field gagal
        if (Object.keys(errors).length > 0) {
            setError('Terdapat data wajib yang belum diisi atau format tidak benar.');
            return false;
        }

        setError(''); // Clear general error if validation succeeds
        return true;
    };
    // END: FUNGSI VALIDASI BARU UNTUK STEP 1

    const handleSubmit = async (e) => {
        e.preventDefault();
        
        if (step === 1) {
            // Ganti logic lama dengan validasi baru
            if (!validateStep1()) {
                return;
            }
            setStep(2);
        ...
        try {
            ...
            // 2. Create profile with is_approved: false
            if (authData.user) {
                const profileData = {
                    id: authData.user.id,
                    email: formData.email.trim().toLowerCase(),
                    ...
                    // KUNCI UTAMA: Tentukan status persetujuan awal
                    is_approved: false, 
                };
                                ...
                                <div className="space-y-4">
                                    <Alert className="border-green-500/50 bg-green-500/10">
                                        ...
                                        <AlertDescription className="text-green-600">
                                            Registrasi berhasil! Silakan cek email Anda untuk verifikasi, **setelah diverifikasi, admin akan meninjau akun Anda**.
                                        </AlertDescription>
                                    </Alert>
                                            ...
                                            <>
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Input 
                                                        ...
                                                        disabled={loading}
                                                        // Hapus required bawaan agar validasi custom tampil
                                                        // required
                                                        aria-invalid={!!fieldErrors.fullName}
                                                    />
                                                    {/* START: FEEDBACK VALIDASI */}
                                                    {fieldErrors.fullName && ( 
                                                        <p className="text-sm text-red-500">{fieldErrors.fullName}</p>
                                                    )}
                                                    {/* END: FEEDBACK VALIDASI */}
                                                </div>
                                                ...
                                                <Button 
                                                    type="submit"
                                                    // Tombol diaktifkan karena validasi dilakukan di handleSubmit (menghindari !isStep1Valid lama)
                                                    disabled={loading} 
                                                    className="w-full"
                                                    size="lg"
                                                >
